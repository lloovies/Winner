...
contract WINNER_S is SeroInterface, Ownable {
    ...
    mapping(uint8 => Winner3) private smallWinnerMap;
    ...
    function getThird() public view returns(uint256, uint256, uint256) {
        return (smallWinnerMap[2].investCode_1, smallWinnerMap[2].investCode_2, smallWinnerMap[2].investCode_3);
    }
    ...
    function sendSmallPrize() public onlyOwner {
        require(canPlay);
        canPlay = false;

        require(sero_send_token(smallWinnerMap[0].investAddress, SERO_CURRENCY, winnerPool));
        require(sero_send_token(smallWinnerMap[0].investAddress, TOKEN_CURRENCY, 1e20));
        if (luckyStarAddr != 0) {
            require(!canPlayByLucky);
        ...
        delete smallWinnerMap[0];
        delete smallWinnerMap[1];
        delete smallWinnerMap[2];
    ...
    function insertBets(uint256 loopTimes, address _addr) private {
        for (uint256 i = 0; i < loopTimes; i++) {
            ...
            updateSmallWinner(betsMap[index].investCode_1, betsMap[index].investCode_2, betsMap[index].investCode_3, betsMap[index].investTimestamp, betsMap[index].investAddress);
            index += 1;
        }
    }

    function updateSmallWinner(uint256 code1, uint256 code2, uint256 code3, uint256 timestamp, address _addr) private {
        uint256 newDummy = calculateDummy(code1, code2, code3);
        if (newDummy > smallWinnerMap[0].dummy) {
            smallWinnerMap[2] = smallWinnerMap[1];
            smallWinnerMap[1] = smallWinnerMap[0];
            smallWinnerMap[0].investTimestamp = timestamp;
            smallWinnerMap[0].investCode_1 = code1;
            smallWinnerMap[0].investCode_2 = code2;
            smallWinnerMap[0].investCode_3 = code3;
            smallWinnerMap[0].investAddress = _addr;
            smallWinnerMap[0].dummy = newDummy;
        } else if (newDummy > smallWinnerMap[1].dummy && newDummy <= smallWinnerMap[0].dummy) {
            smallWinnerMap[2] = smallWinnerMap[1];
            smallWinnerMap[1].investTimestamp = timestamp;
            smallWinnerMap[1].investCode_1 = code1;
            smallWinnerMap[1].investCode_2 = code2;
            smallWinnerMap[1].investCode_3 = code3;
            smallWinnerMap[1].investAddress = _addr;
            smallWinnerMap[1].dummy = newDummy;
        } else if (newDummy > smallWinnerMap[2].dummy && newDummy <= smallWinnerMap[1].dummy) {
            smallWinnerMap[2].investTimestamp = timestamp;
            smallWinnerMap[2].investCode_1 = code1;
            smallWinnerMap[2].investCode_2 = code2;
            smallWinnerMap[2].investCode_3 = code3;
            smallWinnerMap[2].investAddress = _addr;
            smallWinnerMap[2].dummy = newDummy;
        }
    }
    ...
    function betSmall() public payable returns (bool) {
        require(canPlay);
        require(Utils._stringEq(SERO_CURRENCY, sero_msg_currency()));
        require(!Utils.isContract(msg.sender));
        require(msg.value >= 1e19 && msg.value <= 1e20);
        uint256 count = msg.value.div(1e19);
